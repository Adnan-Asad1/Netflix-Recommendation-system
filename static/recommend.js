  ...
  $.ajax({
    ...
    success: async function (movie) {
      if (movie.results.length < 1) {
        ...
        await movie_recs(movie_title, movie_id, my_api_key);
      }
    },
    error: function () {
  ...
  $.ajax({
    ...
    success: async function (recs) {
      if (recs === "Sorry! The movie you requested is not in our database. Please check the spelling or try with some other movies") {
        ...
        await get_movie_details(movie_id, my_api_key, arr, movie_title);
      }
    },
    error: function () {
...
async function get_movie_details(movie_id, my_api_key, arr, movie_title) {
  $.ajax({
    ...
    success: async function (movie_details) {
      await show_details(movie_details, arr, movie_title, my_api_key, movie_id);
    },
    error: function () {
      alert("API Error!");
      $("#loader").delay(500).fadeOut();
